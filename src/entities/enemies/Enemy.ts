...
export abstract class Enemy extends Character {
    ...
    player: Character ; // Référence au joueur
    ...
    attackRange : number ; // Portée d'attaque de l'ennemi
    isAttacking : boolean;
    attackCooldown : boolean; // Cooldown d'attaque de l'ennemi
    ...
    // Constructeur 
    constructor(name : string, mesh: AbstractMesh,animationGroups: AnimationGroup[], initialPosition: Vector3, player: Character,navPlugin: RecastJSPlugin, patrolAreaRadius: number,patrolPoints: Vector3[], sightRange: number,scene: Scene) {
    ...
    public update(deltaTime: number): void {
        switch (this.state) {
            ...
            case EnemyState.STATE_ATTACK:
                ...
                break;
            case EnemyState.STATE_GET_SMALL_DAMAGE:
                this.takeDamage(10);
                break;
            case EnemyState.STATE_GET_BIG_DAMAGE:
                this.takeDamage(20);
                break;
            ...
            case EnemyState.STATE_RESPAWN:
                ...
                break; 
            default:
                console.error("Unknown enemy state:", this.state);
                console.debug("[Enemy] " + this.name + " is in an unknown state: " + this.state);
                break;
        }
   ...
   /*  public chasePlayer(player: Character, deltaTime: number): void {
    ...
    public die(): void {
        this.animationManager.play("Die", false, () => {
            ...
            , 1000); // Wait for 1 second before disposing the mesh
        });
    }
