    ...
    import {
        ...
        MeshBuilder, Color3, StandardMaterial
    } from "@babylonjs/core";
    ...
    import { EnemyHealthUIManager } from "./ui/EnemyHealthUIManager"; 
    import { StartMenuUI } from "./ui/StartMenuUI";

    class App {
        private ui: GameUI | null = null;
        private enemyUIManager: EnemyHealthUIManager | null = null; 
        ...
        constructor() {
            ...
            window.addEventListener("click", async () => {
                let audioEngine = Engine.audioEngine;
                ...
                try {
                    ...
                    console.warn("Failed to unlock audio engine:", err);
                }
            }, { once: true });
            ...
            assetsManager.loadAllAssets(async () => {
                ...
                const navParams = {
                    ...
                    walkableHeight: 1, walkableClimb: 1, walkableRadius: 1,
                    maxEdgeLen: 12, maxSimplificationError: 1.3,
                    minRegionArea: 8, mergeRegionArea: 20,
                    maxVertsPerPoly: 6, detailSampleDist: 6,
                ...
                const chestMonster = new BeholderMonster(
                    "chestMonster",
                    chestMonsterMesh,
                    chestMonsterAnimationGroups,
                    new Vector3(0, 0, 0),
                    knight,
                    navPlugin,
                    8,
                    [new Vector3(5, 0, 0), new Vector3(0, 0, 5), new Vector3(-5, 0, 0), new Vector3(0, 0, -5)],
                    10,
                    this.scene
                );
                ...
                this.enemyUIManager = new EnemyHealthUIManager(this.scene); 
                this.enemyUIManager.addEnemy(chestMonster);   
                
                const startMenu = new StartMenuUI(
                    this.scene,
                    () => {
                        knight.enableControls();       // Unfreeze controls
                        this.ui!.show();               // Show player HUD
                    },
                    () => alert("Use WASD to move, click to attack, Shift to dash."),
                    () => alert("Settings menu coming soon!"),
                    () => alert("Game by Your Name. Assets from XYZ sources."),
                    () => location.reload()
                );
                this.ui!.hide();  // Hide HUD until game starts
